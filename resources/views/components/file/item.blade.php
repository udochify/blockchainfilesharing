...
<div id="ajax-file{{$file->id}}" {{ $attributes->merge(['class'=>'flex flex-row items-end p-1 border-b border-solid border-b-gray-200']) }}>
    ...
    <div class="flex flex-col items-start ml-2">
        <div class="flex flex-row">
            ...
            <div id="share{{$file->id}}" title="share file" class="share{{$file->id}} inline-block px-2 mr-1 text-xs leading-[1.125rem] cursor-pointer hover:bg-green-500 font-semibold rounded-full bg-green-400 text-white ajax-btn">
                share
            </div>
            <div id="unshare{{$file->id}}" title="unshare file" class="unshare{{$file->id}} inline-block px-2 mr-1 text-xs leading-[1.125rem] cursor-pointer hover:bg-green-500 font-semibold rounded-full bg-green-400 text-white ajax-btn">
                unshare
            </div>
        ...
        <form action="{{ route('files.share', $file->id) }}" class="share{{$file->id}} hidden">@csrf</form>
        <form action="{{ route('files.unshare', $file->id) }}" class="unshare{{$file->id}} hidden">@csrf</form>
    </div>
